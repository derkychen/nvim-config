...
local function dependency_graph(client)
  client:exec_cmd({ command = "texlab.showDependencyGraph" }, { bufnr = 0 },
    function(err, result)
      if err then
...
local function command_factory(cmd)
  ...
  return function(client, bufnr)
    return client:exec_cmd({
      ...
      if err then
        vim.notify(("Failed to clean %s files: %s"):format(cmd, err.message),
          vim.log.levels.ERROR)
      else
        vim.notify(("Command %s executed successfully"):format(cmd),
          vim.log.levels.INFO)
      end
    end)
  end
